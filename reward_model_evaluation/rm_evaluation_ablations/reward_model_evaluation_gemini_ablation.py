...
class VertexAIBatchProcessor:
    """Handle Google GenAI batch processing operations"""
    ...
    async def call_genai_model(self, prompt_text: str, model_name: str, 
        ...
        """
        Make a single API call to Google GenAI.
        
        Note: Input context is not artificially limited to preserve experimental validity.
        The model will handle context limits naturally.
        """
        try:
...
async def evaluate_preference_pair_all_settings_batch(preference_pairs, ablation_data_manager, 
                                                     vertex_processor, args, 
                                                     ablation_conflict_sets=None):
    """
    Evaluate multiple preference pairs using all ablation settings in batch.
    ...
    Args:
        ...
        vertex_processor: VertexAIBatchProcessor instance
    ...
    for preference_pair in preference_pairs:
        ...
        try:
            batch_results = await vertex_processor.submit_batch_job(
                prompts_and_settings, args.model_name, 
                args.max_tokens, args.temperature, args.seed, args.max_concurrent
            )
            
        except Exception as e:
            print(f"Error in batch processing for preference {preference_id}: {e}")
            # Create error responses for all settings
            batch_results = {"results": []}
            for _, _, setting_name in prompts_and_settings:
        ...
        # Process results by custom_id
        results_by_id = {r["custom_id"]: r for r in batch_results["results"]}
        
        # Add all responses and correctness
        for _, _, setting_name in prompts_and_settings:
            if setting_name in results_by_id:
                batch_result = results_by_id[setting_name]
...
def get_question_ids_for_group(ablation_data_manager: AblationDataManager, question_group: str) -> List[str]:
    """Get question IDs for the specified question group"""
    if question_group == "core_wvs_only":
        return ablation_data_manager.available_key_wvs_questions.copy()
    elif question_group == "available_key_wvs":
        return ablation_data_manager.available_key_wvs_questions.copy()
    else:
        raise ValueError(f"Invalid question group: {question_group}. Choose from: core_wvs_only, available_key_wvs")

def estimate_evaluation_size(args, ablation_data_manager, question_group):
    """Estimate evaluation size"""
    # Get question IDs for selected group
    question_ids = get_question_ids_for_group(ablation_data_manager, question_group)
    
    # Estimate preference pairs (this is approximate)
    estimated_pairs = len(question_ids) * 50  # Rough estimate based on available user profiles
    
    if args.max_evaluations:
        estimated_pairs = min(estimated_pairs, args.max_evaluations)
    
    # Settings per evaluation
    settings_per_eval = 4  # 4 ablation settings
    if args.all_settings:  # Add COT settings
        settings_per_eval = 8
    
    total_requests = estimated_pairs * settings_per_eval
    
    print(f"\nEvaluation Size Estimation")
    print(f"Estimated preference pairs: {estimated_pairs:,}")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Gemini WVS Ablation Evaluation for Reward Model Evaluation v3")
    
    # Model arguments
    parser.add_argument("--model_name", type=str, required=True,
                       help="Gemini model name (e.g., gemini-1.5-pro, gemini-1.5-flash, gemini-1.0-pro)")
    ...
    parser.add_argument("--seed", type=int, default=0,
                       help="Random seed for model generation (if supported)")
    parser.add_argument("--max_concurrent", type=int, default=10,
                       help="Maximum concurrent requests to Gemini API")
    ...
    if not args.output_file:
        ...
        args.output_file = f"{model_name}_gemini_wvs_ablation_conflict_only_{settings_suffix}_results.json"
    
    print("=== Gemini WVS Ablation Evaluation for Value-Style Conflicts ===")
    ...
    question_ids = get_question_ids_for_group(ablation_data_manager, args.questions)
    ...
    # Estimate evaluation size
    estimated_evaluations = estimate_evaluation_size(args, ablation_data_manager, args.questions)
    ...
    print(f"\n=== Initializing Vertex AI Processor ===")
    try:
        vertex_processor = VertexAIBatchProcessor(args.project_id, args.location)
        
        print("Vertex AI processor initialized successfully")
        print(f"Model: {args.model_name}")
        print(f"Project: {args.project_id}")
        print(f"Location: {args.location}")
        
    except Exception as e:
        print(f"Error initializing Vertex AI processor: {e}")
        return
    ...
    output_data = {
        'metadata': {
            ...
            'evaluation_type': 'gemini_wvs_ablation_conflict_only',
    ...
    print(f"Processing {len(conflict_preference_pairs)} conflict cases in batches of {args.save_interval}")
    
    try:
        start_time = time.time()
        
        # Process evaluations in batches
        for batch_start in range(0, len(conflict_preference_pairs), args.save_interval):
            batch_end = min(batch_start + args.save_interval, len(conflict_preference_pairs))
            batch_pairs = conflict_preference_pairs[batch_start:batch_end]
            
            print(f"\nProcessing batch {batch_start//args.save_interval + 1}/{(len(conflict_preference_pairs)-1)//args.save_interval + 1} "
                  f"(items {batch_start+1}-{batch_end}/{len(conflict_preference_pairs)})")
            
            try:
                # Evaluate this batch with all settings using batch processing
                batch_results = await evaluate_preference_pair_all_settings_batch(
                    batch_pairs, ablation_data_manager, vertex_processor, args, ablation_conflict_sets
                )
            ...
            if elapsed_time > 0:
                ...
                if evaluation_results:
                    ...
                    print(f"Completed batch {batch_start//args.save_interval + 1}. "
...
""
    ...
    print(f"\n=== GEMINI ABLATION EVALUATION SUMMARY ===")
    print(f"Evaluation type: Gemini WVS Ablation (Value-Style Conflicts Only)")
    ...
    print("\nGemini ablation evaluation completed!")
